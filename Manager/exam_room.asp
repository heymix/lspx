...
function statusPost(status,type,checkType){
	...
	if (status=="4"){
		...
		$.post(	"../Public/exam_roomStatus.asp?status="+status+"&checkType="+checkType+"&operateType="+type, 
				$("#listForm").serialize(), 
				function(data,st){
...
With ors
	...
	.Sql="ER.ID,ER.TEST_ID,ER.EXA_ID,ER.T_ROOM,ER.T_ROOM_ORDER,ER.T_MAX_NUM,ER.CHECKED$[T_EXA_ROOM] AS ER Left join T_EXA AS E ON ER.EXA_ID=E.ID $1=1 And ER.TEST_ID="&TEST_ID&" and EXA_ID="&sys_exam_id&" "&findKey&"$ER.ID asc$ER.ID" '字段,表,条件(不需要where),排序(不需要需要ORDER BY),主键ID
End With
...
Else     
	...
	%>    
          <tr>
            ...
            <td height="18" ><%If iRs(6,i)=1 Then
								Response.write "不可操作"
							  Else %><img src="../Images/edit.gif" width="14" height="14" /><a href="javascript:void(0)" onClick="window.location.href='exam_room_add.asp?id=<%=iRs(0,i)%>'">[编辑]</a><%
							  End If
							  %></td>
            </tr>
