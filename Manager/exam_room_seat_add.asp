...
If EMAIL<>"" and not sqlInjection(EMAIL) Then
	findKey=findKey&" and EMAIL like'%"&EMAIL&"%'"
End If
        ...
        <label for="SCHOOL_ID">所在学校:</label>
            <select id="SCHOOL_ID" name="SCHOOL_ID" >
                ...
                <%
					...
					sql="select ID,SCHOOL from T_SCHOOL where CHECKED=1 and ID in(select SCHOOL_ID from T_EXA_SCHOOL_RELATION  where EXA_ID in(select EXA_ID from T_TEST_EXA_RELATION AS TE left join T_EXA AS T ON TE.EXA_ID=T.ID  where TEST_ID="&test_id&"))  order by SCHOOL asc"
					response.write sql
...
With ors
	...
	.Sql="USER_ID,SCHOOL_ID,SCHOOL,GENDER,REAL_NAME,AGE,EDUCATION,TEL,MOBILE,EMAIL,LAST_LOGIN_TIME,CHECKED,LOCKED,IS_MAKEUP,TRCHECKED$V_EXAMINEE_TEST$TRCHECKED=1 and TEST_ID="&test_id&"  "&findKey&"$USER_ID desc$USER_ID" '字段,表,条件(不需要where),排序(不需要需要ORDER BY),主键ID
End With 
