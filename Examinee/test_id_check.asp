     ...
     <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
       <tr>
         ...
         <td bgcolor="#f3ffe3"  class="tdStyle"><div  class="choiceTest">
           <form method="post" id="choiceTestForm" name="choiceTestForm"  title="学校管理" >
             <label for="TEST_ID">管理的考试:</label>
             <select id="TEST_ID" name="TEST_ID" style="width:400px"  >
               <option value="0">选择考试</option>
               <%
					set rs=server.createobject("adodb.recordset")
					sql="select ID,TITLE,TEST_CATE from T_TEST as main where CHECKED=1 and IS_OVER=0 and (select SCHOOL_ID from T_EXAMINEE where USER_ID='"&SYS_USER_ID&"') in (select SCHOOL_ID  from T_EXA_SCHOOL_RELATION where EXA_ID in (select EXA_ID from T_TEST_EXA_RELATION as TE where main.ID=TE.TEST_ID))"
