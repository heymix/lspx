			...
			If IS_MAKEUP="0" Then
				...
				do while not (rs.eof or err)
					If rs("IS_PRATICE")=1 Then
						conn.execute("insert into T_TOPIC_RESULT (USER_ID,TEST_ID,TOPIC_ID,IS_MAKEUP,RESULT) values('"&EXAMINEE_USER_ID&"',"&myTEST_ID&","&rs("TOPIC_ID")&",1,1)")
					Else
						conn.execute("insert into T_TOPIC_RESULT (USER_ID,TEST_ID,TOPIC_ID,IS_MAKEUP) values('"&EXAMINEE_USER_ID&"',"&myTEST_ID&","&rs("TOPIC_ID")&",1)")
					End If
				rs.movenext
				loop
				rs.close 
				...
				for i=0 to Ubound(udID)
					If isNumeric(udID(i)) Then
						...
						If IS_PRATICE=1 Then
							conn.execute("insert into T_TOPIC_RESULT (USER_ID,TEST_ID,TOPIC_ID,IS_MAKEUP,RESULT) values('"&EXAMINEE_USER_ID&"',"&myTEST_ID&","&udID(i)&",1,1)")
						Else
