	...
	If PURVIEW="0" Then
		...
		for i=0 to Ubound(udID)
			If isNumeric(udID(i)) Then
				If oStatus="5" Then 
					conn.execute("DELETE T_TEST_TOPIC_RELATION where TEST_ID="&TEST_ID&" AND TOPIC_ID="&udID(i))
					operateCon="从考试("&test_id&":"&test_title&")中移除科目"
				End If
				If oStatus="6" Then 
					theSame=conn.execute("select count(*) from T_TEST_TOPIC_RELATION where TEST_ID="&TEST_ID&" AND TOPIC_ID="&udID(i))(0)			
					If theSame=0 Then
						conn.execute("insert into T_TEST_TOPIC_RELATION (TEST_ID,TOPIC_ID)values("&TEST_ID&","&udID(i)&")")
					End If
					operateCon="添加科目到考试("&test_id&":"&test_title&")"
				End If
			End If
		next
		if Err.Number = 0 Then
			operateLog "current_topic.asp",operateCon&"："&subBox
